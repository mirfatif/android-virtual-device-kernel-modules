...
static int dxgk_presentvirtual2(struct dxgprocess *process, void *__user inargs)
{
	...
	layer_fds = vzalloc(sizeof(int) * args.layer_memory_fd_count);
	if (layer_fds == NULL) {
		ret = -ENOMEM;
		goto cleanup;
	}
	...
	ret = copy_from_user(layer_fds, &args.layer_memory_fd, sizeof(int) * args.layer_memory_fd_count);
	...
	for(layer_idx = 0; layer_idx < args.layer_memory_fd_count; layer_idx++) {
		ret = get_resource_host_nthandle(layer_fds[layer_idx], &layer_memory_nthandle[layer_idx]);
		if (ret) {
