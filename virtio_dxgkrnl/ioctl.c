...
static int dxgsyncobj_release(struct inode *inode, struct file *file) {
  ...
  if (syncobj->host_shared_handle_nt_reference == 0) {
    if (syncobj->host_shared_handle_nt.v) {
      dxgvmb_send_destroy_nt_shared_object(syncobj->host_shared_handle_nt);
...
static int dxgsharedresource_release(struct inode *inode, struct file *file) {
  ...
  if (resource->host_shared_handle_nt_reference == 0) {
    if (resource->host_shared_handle_nt.v) {
      dxgvmb_send_destroy_nt_shared_object(resource->host_shared_handle_nt);
...
static int dxgk_open_adapter_from_luid(struct dxgprocess *process,
  ...
  list_for_each_entry(entry, &dxgglobal->adapter_list_head,
    ...
    if (dxgadapter_acquire_lock_shared(entry) == 0) {
      dev_dbg(dxgglobaldev, "Compare luids: %d:%d  %d:%d", entry->luid.b,
              entry->luid.a, args.adapter_luid.b, args.adapter_luid.a);
      if (*(u64 *)&entry->luid == *(u64 *)&args.adapter_luid) {
        ret = dxgprocess_open_adapter(process, entry, &args.adapter_handle);

        if (ret >= 0) {
          ret = copy_to_user(&result->adapter_handle, &args.adapter_handle,
  ...
  if (args.adapter_handle.v == 0) ret = -EINVAL;

cleanup:

  if (ret < 0) dxgprocess_close_adapter(process, args.adapter_handle);
...
static int dxgkp_enum_adapters(struct dxgprocess *process,
  ...
  if (info_out == NULL || adapter_count_max == 0) {
    ...
    ret =
        copy_to_user(adapter_count_out, &dxgglobal->num_adapters, sizeof(u32));
    if (ret) {
  ...
  list_for_each_entry(entry, &dxgglobal->adapter_list_head,
    ...
    if (dxgadapter_acquire_lock_shared(entry) == 0) {
      ...
      ret = dxgprocess_open_adapter(process, entry, &inf->adapter_handle);
      if (ret >= 0) {
        inf->adapter_luid = entry->luid;
        adapters[adapter_count] = entry;
        dev_dbg(dxgglobaldev, "adapter: %x %x:%x", inf->adapter_handle.v,
                inf->adapter_luid.b, inf->adapter_luid.a);
        adapter_count++;
      }
      dxgadapter_release_lock_shared(entry);
    }
  ...
  ret = copy_to_user(adapter_count_out, &adapter_count, sizeof(adapter_count));
  if (ret) {
...
cleanup:
  ...
  if (info) {
    for (i = 0; i < adapter_count; i++)
      dxgprocess_close_adapter(process, info[i].adapter_handle);
  }
success:
  if (info) vfree(info);
  if (adapters) vfree(adapters);
...
static int dxgsharedresource_seal(struct dxgsharedresource *shared_resource) {
  ...
  if (!list_empty(&shared_resource->resource_list_head)) {
    resource = list_first_entry(&shared_resource->resource_list_head,
                                struct dxgresource, shared_resource_list_entry);
    ...
    list_for_each_entry(alloc, &resource->alloc_list_head, alloc_list_entry) {
      ...
      if (alloc_data_size) {
        ...
        shared_resource->alloc_private_data_sizes[i] = alloc_data_size;
        memcpy(private_data, alloc->priv_drv_data->data, alloc_data_size);
...
static int dxgk_enum_adapters(struct dxgprocess *process, void *__user inargs) {
  ...
  list_for_each_entry(entry, &dxgglobal->adapter_list_head,
    ...
    if (dxgadapter_acquire_lock_shared(entry) == 0) {
      ...
      if (ret >= 0) {
        inf->adapter_luid = entry->luid;
        adapters[adapter_count] = entry;
        dev_dbg(dxgglobaldev, "adapter: %x %llx", inf->adapter_handle.v,
  ...
  ret = copy_to_user(args.adapters, info, sizeof(info[0]) * args.num_adapters);
  if (ret) {
...
static int dxgk_enum_adapters3(struct dxgprocess *process,
                               void *__user inargs) {
  ...
  ret = dxgkp_enum_adapters(
      process, args.filter, args.adapter_count, args.adapters,
      &((struct d3dkmt_enumadapters3 *)inargs)->adapter_count);
...
static int dxgk_close_adapter(struct dxgprocess *process, void *__user inargs) {
  ...
  if (ret < 0) pr_err("%s failed", __func__);
...
static int dxgk_query_adapter_info(struct dxgprocess *process,
  ...
  dev_dbg(dxgglobaldev, "Type: %d Size: %x", args.type, args.private_data_size);
...
cleanup:
  ...
  if (adapter) kref_put(&adapter->adapter_kref, dxgadapter_release);
...
static int dxgk_create_device(struct dxgprocess *process, void *__user inargs) {
  ...
  if (host_device_handle.v) {
    ret = copy_to_user(&((struct d3dkmt_createdevice *)inargs)->device,
                       &host_device_handle, sizeof(struct d3dkmthandle));
    if (ret) {
    ...
    ret = hmgrtable_assign_handle(&process->handle_table, device,
                                  HMGRENTRY_TYPE_DXGDEVICE, host_device_handle);
    if (ret >= 0) {
...
cleanup:
  ...
  if (ret < 0) {
    if (host_device_handle.v)
      dxgvmb_send_destroy_device(adapter, process, host_device_handle);
    if (device) dxgdevice_destroy(device);
  }

  if (adapter_locked) dxgadapter_release_lock_shared(adapter);
...
static int dxgk_destroy_device(struct dxgprocess *process,
                               void *__user inargs) {
  ...
  device = hmgrtable_get_object_by_type(
      &process->handle_table, HMGRENTRY_TYPE_DXGDEVICE, args.device, true);
  if (device) {
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGDEVICE,
                          args.device);
...
static int dxgk_create_context_virtual(struct dxgprocess *process,
  ...
  host_context_handle = dxgvmb_send_create_context(adapter, process, &args);
  if (host_context_handle.v) {
    hmgrtable_lock(&process->handle_table, DXGLOCK_EXCL);
    ret =
        hmgrtable_assign_handle(&process->handle_table, context,
                                HMGRENTRY_TYPE_DXGCONTEXT, host_context_handle);
    if (ret >= 0) context->handle = host_context_handle;
    hmgrtable_unlock(&process->handle_table, DXGLOCK_EXCL);
    if (ret < 0) goto cleanup;
    ret = copy_to_user(&((struct d3dkmt_createcontextvirtual *)inargs)->context,
                       &host_context_handle, sizeof(struct d3dkmthandle));
    if (ret) {
...
cleanup:
  ...
  if (ret < 0) {
    if (host_context_handle.v) {
      dxgvmb_send_destroy_context(adapter, process, host_context_handle);
    }
    if (context) dxgcontext_destroy_safe(process, context);
  }

  if (adapter) dxgadapter_release_lock_shared(adapter);

  if (device) {
    if (device_lock_acquired) dxgdevice_release_lock_shared(device);
...
static int dxgk_destroy_context(struct dxgprocess *process,
                                void *__user inargs) {
  ...
  hmgrtable_lock(&process->handle_table, DXGLOCK_EXCL);
  context = hmgrtable_get_object_by_type(
      &process->handle_table, HMGRENTRY_TYPE_DXGCONTEXT, args.context, true);
  if (context) {
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGCONTEXT,
                          args.context);
    context->handle.v = 0;
...
cleanup:
  ...
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
static int dxgk_create_hwqueue(struct dxgprocess *process,
                               void *__user inargs) {
  ...
  ret = dxgvmb_send_create_hwqueue(process, adapter, &args, inargs, hwqueue);

cleanup:

  if (ret < 0 && hwqueue) dxghwqueue_destroy(process, hwqueue);
...
static int dxgk_destroy_hwqueue(struct dxgprocess *process,
  ...
  hmgrtable_lock(&process->handle_table, DXGLOCK_EXCL);
  hwqueue = hmgrtable_get_object_by_type(
      &process->handle_table, HMGRENTRY_TYPE_DXGHWQUEUE, args.queue, true);
  if (hwqueue) {
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGHWQUEUE,
                          args.queue);
    hwqueue->handle.v = 0;
...
static int dxgk_create_paging_queue(struct dxgprocess *process,
  ...
  if (ret >= 0) {
    ...
    ret = hmgrtable_assign_handle(&process->handle_table, pqueue,
                                  HMGRENTRY_TYPE_DXGPAGINGQUEUE, host_handle);
    if (ret >= 0) {
      pqueue->handle = host_handle;
      ret = hmgrtable_assign_handle(&process->handle_table, NULL,
                                    HMGRENTRY_TYPE_MONITOREDFENCE,
                                    args.sync_object);
      if (ret >= 0) pqueue->syncobj_handle = args.sync_object;
    }
    hmgrtable_unlock(&process->handle_table, DXGLOCK_EXCL);
...
cleanup:
  ...
  if (ret < 0) {
    if (pqueue) dxgpagingqueue_destroy(pqueue);
    if (host_handle.v)
      dxgvmb_send_destroy_paging_queue(process, adapter, host_handle);
  }
...
static int dxgk_destroy_paging_queue(struct dxgprocess *process,
  ...
  if (paging_queue) {
    ...
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGPAGINGQUEUE,
                          args.paging_queue);
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_MONITOREDFENCE,
  ...
  ret = dxgvmb_send_destroy_paging_queue(process, adapter, args.paging_queue);
...
static int get_standard_alloc_priv_data(
    struct dxgdevice *device,
    struct d3dkmt_createstandardallocation *alloc_info,
    u32 *standard_alloc_priv_data_size, void **standard_alloc_priv_data,
    u32 *standard_res_priv_data_size, void **standard_res_priv_data) {
  int ret;
  ...
  *standard_alloc_priv_data_size = 0;
  ret = dxgvmb_send_get_stdalloc_data(
      device, _D3DKMDT_STANDARDALLOCATION_GDISURFACE, &gdi_data, 0,
      &priv_data_size, NULL, &res_priv_data_size, NULL);
  if (ret < 0) goto cleanup;
  ...
  if (priv_data == NULL) {
    ret = -ENOMEM;
    pr_err("failed to allocate memory for priv data: %d", priv_data_size);
    goto cleanup;
  }
  ...
  ret = dxgvmb_send_get_stdalloc_data(
      device, _D3DKMDT_STANDARDALLOCATION_GDISURFACE, &gdi_data, 0,
      &priv_data_size, priv_data, &res_priv_data_size, res_priv_data);
  if (ret < 0) goto cleanup;
...
cleanup:
  if (priv_data) vfree(priv_data);
  if (res_priv_data) vfree(res_priv_data);
  if (ret) dev_dbg(dxgglobaldev, "err: %s %d", __func__, ret);
  return ret;
}
...
static int dxgk_create_allocation(struct dxgprocess *process,
  ...
  if (args.alloc_count > D3DKMT_CREATEALLOCATION_MAX || args.alloc_count == 0) {
    pr_err("invalid number of allocations to create");
    ret = -EINVAL;
    goto cleanup;
  }

  alloc_info_size = sizeof(struct d3dddi_allocationinfo) * args.alloc_count;
  alloc_info = vzalloc(alloc_info_size);
  if (alloc_info == NULL) {
    ret = -ENOMEM;
    goto cleanup;
  }
  ret = copy_from_user(alloc_info, args.allocation_info, alloc_info_size);
  if (ret) {
  ...
  for (i = 0; i < args.alloc_count; i++) {
    ...
    if (alloc_info[i].priv_drv_data_size >= DXG_MAX_VM_BUS_PACKET_SIZE) {
      pr_err("private data size is too big: %d %d %ld", i,
             alloc_info[i].priv_drv_data_size, sizeof(alloc_info[0]));
      ret = -EINVAL;
      goto cleanup;
    }
  }
  ...
  if (args.flags.standard_allocation) {
    ...
    ret = copy_from_user(&standard_alloc, args.standard_allocation,
    ...
    if (args.priv_drv_data_size != 0 || args.alloc_count != 1 ||
    ...
    args.priv_drv_data_size = sizeof(struct d3dkmt_createstandardallocation);
  }
  ...
  if (args.flags.standard_allocation) {
    ret = get_standard_alloc_priv_data(
        device, &standard_alloc, &standard_alloc_priv_data_size,
        &standard_alloc_priv_data, &res_priv_data_size, &res_priv_data);
    if (ret < 0) goto cleanup;
  ...
  if (args.flags.create_resource) {
    ...
    resource->private_runtime_handle = args.private_runtime_resource_handle;
    if (args.flags.create_shared) {
      if (!args.flags.nt_security_sharing) {
        dev_err(dxgglobaldev, "%s: nt_security_sharing must be set", __func__);
        ret = -EINVAL;
        goto cleanup;
      }
      ...
      shared_resource->runtime_private_data_size = args.priv_drv_data_size;
      shared_resource->resource_private_data_size = args.priv_drv_data_size;
      ...
      if (args.flags.standard_allocation) {
        shared_resource->resource_private_data = res_priv_data;
        shared_resource->resource_private_data_size = res_priv_data_size;
        res_priv_data = NULL;
      }
      if (args.private_runtime_data_size) {
        shared_resource->runtime_private_data =
            vzalloc(args.private_runtime_data_size);
        if (shared_resource->runtime_private_data == NULL) {
          ret = -ENOMEM;
          goto cleanup;
        }
        ret = copy_from_user(shared_resource->runtime_private_data,
      ...
      if (args.priv_drv_data_size && !args.flags.standard_allocation) {
        shared_resource->resource_private_data =
            vzalloc(args.priv_drv_data_size);
        if (shared_resource->resource_private_data == NULL) {
          ret = -ENOMEM;
          goto cleanup;
        }
        ret = copy_from_user(shared_resource->resource_private_data,
                             args.priv_drv_data, args.priv_drv_data_size);
        if (ret) {
          pr_err("%s failed to copy res data", __func__);
          ret = -EINVAL;
          goto cleanup;
        }
      }
    ...
    if (args.resource.v) {
      ...
      if (resource == NULL || resource->device != device) {
        pr_err("invalid resource handle %x", args.resource.v);
        ret = -EINVAL;
        goto cleanup;
      }
      if (resource->shared_owner && resource->shared_owner->sealed) {
  ...
  for (i = 0; i < args.alloc_count; i++) {
    ...
    if (alloc_info[i].sysmem && !args.flags.standard_allocation) {
      if ((unsigned long)alloc_info[i].sysmem & (PAGE_SIZE - 1)) {
        pr_err("invalid sysmem alloc %d, %p", i, alloc_info[i].sysmem);
        ret = -EINVAL;
        goto cleanup;
      }
    }
    if ((alloc_info[0].sysmem == NULL) != (alloc_info[i].sysmem == NULL)) {
    ...
    if (args.flags.create_shared) {
      ...
      alloc->priv_drv_data =
          vzalloc(priv_data_size + offsetof(struct privdata, data) - 1);
      if (alloc->priv_drv_data == NULL) {
        ret = -ENOMEM;
        goto cleanup;
      }
      if (args.flags.standard_allocation) {
        memcpy(alloc->priv_drv_data->data, standard_alloc_priv_data,
               standard_alloc_priv_data_size);
        alloc->priv_drv_data->data_size = standard_alloc_priv_data_size;
      } else {
        ret = copy_from_user(alloc->priv_drv_data->data,
                             alloc_info[i].priv_drv_data, priv_data_size);
        if (ret) {
  ...
  ret = dxgvmb_send_create_allocation(process, device, &args, inargs, resource,
                                      dxgalloc, alloc_info, &standard_alloc);
  ...
  if (ret >= 0 && args.flags.create_shared) {
    for (i = 0; i < args.alloc_count; i++) {
      ...
      if (args.flags.standard_allocation) {
        memcpy(dxgalloc[i]->priv_drv_data->data, standard_alloc_priv_data,
               standard_alloc_priv_data_size);
        dxgalloc[i]->priv_drv_data->data_size = standard_alloc_priv_data_size;
      } else {
        ret = copy_from_user(dxgalloc[i]->priv_drv_data->data,
                             alloc_info[i].priv_drv_data, priv_data_size);
        if (ret) {
          pr_err("%s failed to copy priv data", __func__);
          ret = -EINVAL;
        ...
        dxgalloc[i]->priv_drv_data->data_size = priv_data_size;
      }
    }
  }
cleanup:
  ...
  if (ret < 0) {
    if (dxgalloc) {
      for (i = 0; i < args.alloc_count; i++) {
        if (dxgalloc[i]) dxgallocation_destroy(dxgalloc[i]);
      }
    }
    if (resource && args.flags.create_resource) {
      if (shared_resource) {
        dxgsharedresource_remove_resource(shared_resource, resource);
      }
      dxgresource_destroy(resource);
    }
  }
  if (shared_resource)
    kref_put(&shared_resource->sresource_kref, dxgsharedresource_destroy);
  if (dxgalloc) vfree(dxgalloc);
  if (standard_alloc_priv_data) vfree(standard_alloc_priv_data);
  if (res_priv_data) vfree(res_priv_data);
  if (alloc_info) vfree(alloc_info);
...
int validate_alloc(struct dxgallocation *alloc0, struct dxgallocation *alloc,
                   struct dxgdevice *device, struct d3dkmthandle alloc_handle) {
  u32 fail_reason;
  ...
  return 0;
cleanup:
  pr_err("Alloc validation failed: reason: %d %x", fail_reason, alloc_handle.v);
  return -EINVAL;
}
...
static int dxgk_destroy_allocation(struct dxgprocess *process,
  ...
  if (args.alloc_count) {
    u32 handle_size = sizeof(struct d3dkmthandle) * args.alloc_count;
    ...
    allocs = vzalloc(sizeof(struct dxgallocation *) * args.alloc_count);
    if (allocs == NULL) {
      ret = -ENOMEM;
      goto cleanup;
    }
    ret = copy_from_user(alloc_handles, args.allocations, handle_size);
    if (ret) {
  ...
  if (args.alloc_count) {
    ...
    for (i = 0; i < args.alloc_count; i++) {
      allocs[i] = hmgrtable_get_object_by_type(&process->handle_table,
                                               HMGRENTRY_TYPE_DXGALLOCATION,
                                               alloc_handles[i], true);
      ret = validate_alloc(allocs[0], allocs[i], device, alloc_handles[i]);
      if (ret < 0) {
    ...
    dxgprocess_ht_lock_exclusive_up(process);
    for (i = 0; i < args.alloc_count; i++) dxgallocation_free_handle(allocs[i]);
  } else {
    struct dxgallocation *alloc;
    ...
    if (resource == NULL) {
      ...
      pr_err("Resource belongs to wrong device: %x", args.resource.v);
      ret = -EINVAL;
    } else {
      hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGRESOURCE,
                            args.resource);
  ...
  ret = dxgvmb_send_destroy_allocation(process, device, &args, alloc_handles);
...
cleanup:
  ...
  if (alloc_handles) vfree(alloc_handles);

  if (allocs) vfree(allocs);
...
static int dxgk_make_resident(struct dxgprocess *process, void *__user inargs) {
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGPAGINGQUEUE, args.paging_queue);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  ret2 = copy_to_user(&input->paging_fence_value, &args.paging_fence_value,
                      sizeof(u64));
  if (ret2) {
  ...
  ret2 = copy_to_user(&input->num_bytes_to_trim, &args.num_bytes_to_trim,
                      sizeof(u64));
  if (ret2) {
...
static int dxgk_evict(struct dxgprocess *process, void *__user inargs) {
  ...
  ret = copy_to_user(&input->num_bytes_to_trim, &args.num_bytes_to_trim,
                     sizeof(u64));
  if (ret) {
...
static int dxgk_reclaim_allocations(struct dxgprocess *process,
  ...
  ret = dxgvmb_send_reclaim_allocations(process, adapter, device->handle, &args,
...
static int dxgk_submit_command(struct dxgprocess *process,
                               void *__user inargs) {
  int ret;
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGCONTEXT, args.broadcast_context[0]);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
...
static int dxgk_submit_command_to_hwqueue(struct dxgprocess *process,
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGHWQUEUE, args.hwqueue);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
...
static int dxgk_submit_signal_to_hwqueue(struct dxgprocess *process,
  ...
  ret = copy_from_user(&hwqueue, args.hwqueues, sizeof(struct d3dkmthandle));
  if (ret) {
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGHWQUEUE, hwqueue);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  ret = dxgvmb_send_signal_sync_object(
      process, adapter, args.flags, 0, zerohandle, args.object_count,
      args.objects, args.hwqueue_count, args.hwqueues, args.object_count,
      args.fence_values, NULL, zerohandle, true);
...
static int dxgk_submit_wait_to_hwqueue(struct dxgprocess *process,
  ...
  ret = dxgvmb_send_wait_sync_object_gpu(process, adapter, args.hwqueue,
                                         args.object_count, objects, fences,
                                         false);

cleanup:

  if (objects) vfree(objects);
  if (fences) vfree(fences);
  if (adapter) dxgadapter_release_lock_shared(adapter);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
static int dxgk_reserve_gpu_va(struct dxgprocess *process,
                               void *__user inargs) {
  int ret;
  ...
  if (adapter == NULL) {
    device = dxgprocess_device_by_object_handle(
        process, HMGRENTRY_TYPE_DXGPAGINGQUEUE, args.adapter);
    if (device == NULL) {
      pr_err("invalid adapter or paging queue: 0x%x", args.adapter.v);
      ret = -EINVAL;
      goto cleanup;
    }
    adapter = device->adapter;
...
static int dxgk_create_sync_object(struct dxgprocess *process,
  ...
  if (args.info.type == _D3DDDI_CPU_NOTIFICATION) {
    event = eventfd_ctx_fdget((int)args.info.cpu_notification.event);
    if (IS_ERR(event)) {
  ...
  ret = hmgrtable_assign_handle(&process->handle_table, syncobj,
                                HMGRENTRY_TYPE_DXGSYNCOBJECT, args.sync_object);
  if (ret >= 0) syncobj->handle = args.sync_object;
  hmgrtable_unlock(&process->handle_table, DXGLOCK_EXCL);
...
cleanup:
  ...
  if (ret < 0) {
    ...
    if (event) eventfd_ctx_put(event);
  }
  if (syncobjgbl)
...
static int dxgk_destroy_sync_object(struct dxgprocess *process,
  ...
  if (syncobj) {
    ...
    syncobj->handle.v = 0;
    hmgrtable_free_handle(&process->handle_table, HMGRENTRY_TYPE_DXGSYNCOBJECT,
                          args.sync_object);
  }
  hmgrtable_unlock(&process->handle_table, DXGLOCK_EXCL);
...
static int dxgk_open_sync_object_nt(struct dxgprocess *process,
  ...
  if (!file) {
    pr_err("failed to get file from handle: %llx", args.nt_handle);
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  syncobj =
      dxgsyncobject_create(process, device, adapter, syncobj_fd->type, flags);
  if (syncobj == NULL) {
  ...
  ret = dxgvmb_send_open_sync_object_nt(process, &dxgglobal->channel, &args,
                                        syncobj);
  if (ret < 0) {
...
success:
  ...
  if (file) fput(file);
  if (syncobj) kref_put(&syncobj->syncobj_kref, dxgsyncobject_release);
  if (adapter) dxgadapter_release_lock_shared(adapter);
  if (device_lock_acquired) dxgdevice_release_lock_shared(device);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
static int dxgk_signal_sync_object(struct dxgprocess *process,
  ...
  ret = dxgvmb_send_signal_sync_object(
      process, adapter, args.flags, args.fence.fence_value, args.context,
      args.object_count, in_args->object_array, args.context_count,
      in_args->contexts, fence_count, NULL, (void *)host_event_id, zerohandle,
      true);
...
cleanup:
  ...
  if (ret < 0) {
    if (host_event_added) {
      ...
      host_event =
          (struct dxghosteventcpu *)dxgglobal_get_host_event(host_event_id);
      if (host_event) {
    ...
    if (event) eventfd_ctx_put(event);
    if (host_event) vfree(host_event);
  }
  if (adapter) dxgadapter_release_lock_shared(adapter);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
static int dxgk_signal_sync_object_cpu(struct dxgprocess *process,
  ...
  if (args.object_count == 0 ||
      args.object_count > D3DDDI_MAX_OBJECT_SIGNALED) {
    dev_dbg(dxgglobaldev, "Too many objects: %d", args.object_count);
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  ret = dxgvmb_send_signal_sync_object(
      process, adapter, args.flags, 0, zerohandle, args.object_count,
      args.objects, 0, NULL, args.object_count, args.fence_values, NULL,
      args.device, true);
...
static void dxg_sync_cb(struct dma_fence *fence, struct dma_fence_cb *cb) {
  ...
  if (sync_cb->object_count > 0) {
    ret = dxgvmb_send_signal_sync_object(
        process, adapter, flags, 0, zerohandle, sync_cb->object_count,
        sync_cb->objects, 0, NULL, sync_cb->object_count, sync_cb->fence_values,
        NULL, sync_cb->device_handle, false);
  }
cleanup:
...
static int dxgk_signal_sync_object_from_sync_file(struct dxgprocess *process,
                                                  void *__user inargs) {
  ...
  if (args.object_count == 0 ||
      args.object_count > D3DDDI_MAX_OBJECT_SIGNALED) {
    dev_dbg(dxgglobaldev, "Too few/many objects: %d", args.object_count);
    return -EINVAL;
  }
  ...
  if (args.fd == -1) {
    ret = dxgvmb_send_signal_sync_object(
        process, adapter, flags, 0, zerohandle, args.object_count, args.objects,
        0, NULL, args.object_count, args.fence_values, NULL, args.device, true);
    goto cleanup;
  }
  ...
  sync_cb->objects =
      kcalloc(args.object_count, sizeof(struct d3dkmthandle), GFP_KERNEL);
  ...
  ret = copy_from_user(sync_cb->objects, args.objects,
                       args.object_count * sizeof(struct d3dkmthandle));
  if (ret) {
    pr_err("%s failed to copy objects", __func__);
    // set object count to 0.
    sync_cb->object_count = 0;
  ...
  ret = copy_from_user(sync_cb->fence_values, args.fence_values,
                       args.object_count * sizeof(u64));
  if (ret) {
    pr_err("%s failed to copy fence values", __func__);
    // set object count to 0.
    sync_cb->object_count = 0;
...
static int dxgk_signal_sync_object_gpu(struct dxgprocess *process,
  ...
  struct d3dkmt_signalsynchronizationobjectfromgpu *__user user_args = inargs;
  struct dxgdevice *device = NULL;
  struct dxgadapter *adapter = NULL;
  struct d3dddicb_signalflags flags = {};
  int ret;
  ...
  ret = dxgvmb_send_signal_sync_object(
      process, adapter, flags, 0, zerohandle, args.object_count, args.objects,
      1, &user_args->context, args.object_count, args.monitored_fence_values,
      NULL, zerohandle, true);
...
static int dxgk_signal_sync_object_gpu2(struct dxgprocess *process,
  ...
  if (args.flags.enqueue_cpu_event) {
    if (args.object_count != 0 || args.cpu_event_handle == 0) {
      pr_err("Bad input for EnqueueCpuEvent: %d %lld", args.object_count,
             args.cpu_event_handle);
      ret = -EINVAL;
      goto cleanup;
    }
    ...
    pr_err("Invalid input: %d %d", args.object_count, args.context_count);
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGCONTEXT, context_handle);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  ret = dxgvmb_send_signal_sync_object(
      process, adapter, args.flags, 0, zerohandle, args.object_count,
      args.objects, args.context_count, args.contexts, fence_count, fences,
...
static int dxgk_wait_sync_object(struct dxgprocess *process,
  ...
  ret = dxgvmb_send_wait_sync_object_gpu(process, adapter, args.context,
                                         args.object_count, args.object_array,
...
static int dxgk_wait_sync_object_cpu(struct dxgprocess *process,
  ...
  ret =
      dxgvmb_send_wait_sync_object_cpu(process, adapter, &args, true, event_id);
  if (ret < 0) goto cleanup;
  ...
  if (args.async_event == 0) {
    ...
    if (ret) {
      pr_err("%s: wait_completion_interruptible failed: %d", __func__, ret);
      ret = -ERESTARTSYS;
    }
  }
...
cleanup:
  ...
  if (adapter) dxgadapter_release_lock_shared(adapter);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
  if (host_event.hdr.event_id) dxgglobal_remove_host_event(&host_event.hdr);
  if (ret < 0) {
    if (host_event_added) {
      async_host_event =
          (struct dxghosteventcpu *)dxgglobal_get_host_event(event_id);
      if (async_host_event) {
        if (async_host_event->hdr.event_type == dxghostevent_cpu_event) {
          eventfd_ctx_put(event);
          event = NULL;
    ...
    if (event) eventfd_ctx_put(event);
    if (async_host_event) vfree(async_host_event);
  }
...
static int dxgk_wait_sync_object_gpu(struct dxgprocess *process,
  ...
  if (context) {
    device_handle = context->device_handle;
    syncobj_type =
        hmgrtable_get_object_type(&process->handle_table, objects[0]);
  }
  ...
  if (monitored_fence) {
    ...
    ret = copy_from_user(fences, args.monitored_fence_values, object_size);
    if (ret) {
...
cleanup:
  ...
  if (device) kref_put(&device->device_kref, dxgdevice_release);
  if (objects) vfree(objects);
  if (fences && fences != &args.fence_value) vfree(fences);
...
static int dxgk_lock2(struct dxgprocess *process, void *__user inargs) {
  ...
  if (alloc == NULL) {
    ...
    if (alloc->cpu_address) {
      ret = copy_to_user(&result->data, &alloc->cpu_address, sizeof(args.data));
      if (ret == 0) {
        args.data = alloc->cpu_address;
        if (alloc->cpu_address_mapped) alloc->cpu_address_refcount++;
      } else {
        pr_err("%s Failed to copy cpu address", __func__);
        ret = -EINVAL;
      }
    }
  }
    ...
    if (alloc->cpu_address == NULL) {
      ...
      if (alloc->cpu_address_refcount > 0) {
        ...
        if (alloc->cpu_address_refcount != 0) {
          ...
          dxg_unmap_iospace(alloc->cpu_address, alloc->num_pages << PAGE_SHIFT);
...
static int dxgk_update_alloc_property(struct dxgprocess *process,
  ...
  ret = dxgvmb_send_update_alloc_property(process, adapter, &args, inargs);
...
static int set_context_scheduling_priority(struct dxgprocess *process,
  ...
  device = dxgprocess_device_by_object_handle(
      process, HMGRENTRY_TYPE_DXGCONTEXT, hcontext);
  if (device == NULL) {
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  ret = dxgvmb_send_set_context_sch_priority(process, adapter, hcontext,
                                             priority, in_process);
  if (ret < 0) pr_err("send_set_context_scheduling_priority failed");
cleanup:
  if (adapter) dxgadapter_release_lock_shared(adapter);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
static int dxgk_set_context_scheduling_priority(struct dxgprocess *process,
  ...
  ret = set_context_scheduling_priority(process, args.context, args.priority,
                                        false);
cleanup:
  dev_dbg(dxgglobaldev, "ioctl:%s %s %d", errorstr(ret), __func__, ret);
  return ret;
...
static int get_context_scheduling_priority(struct dxgprocess *process,
                                           struct d3dkmthandle hcontext,
                                           int __user *priority,
                                           bool in_process) {
  int ret;
  ...
  ret = dxgvmb_send_get_context_sch_priority(process, adapter, hcontext, &pri,
                                             in_process);
  if (ret < 0) goto cleanup;
...
static int dxgk_get_context_scheduling_priority(struct dxgprocess *process,
  ...
  ret = get_context_scheduling_priority(process, args.context, &input->priority,
                                        false);
cleanup:
  dev_dbg(dxgglobaldev, "ioctl:%s %s %d", errorstr(ret), __func__, ret);
  return ret;
...
static int dxgk_set_context_process_scheduling_priority(
    struct dxgprocess *process, void *__user inargs) {
...
static int dxgk_get_context_process_scheduling_priority(
    struct dxgprocess *process, void __user *inargs) {
  ...
  ret = get_context_scheduling_priority(
      process, args.context,
      &((struct d3dkmt_getcontextinprocessschedulingpriority *)inargs)
           ->priority,
      true);
cleanup:
  dev_dbg(dxgglobaldev, "ioctl:%s %s %d", errorstr(ret), __func__, ret);
...
static int dxgk_change_vidmem_reservation(struct dxgprocess *process,
  ...
  adapter_locked = true;
  args.adapter.v = 0;
  ret = dxgvmb_send_change_vidmem_reservation(process, adapter, zerohandle,
                                              &args);
...
cleanup:
  ...
  if (adapter_locked) dxgadapter_release_lock_shared(adapter);
  if (adapter) kref_put(&adapter->adapter_kref, dxgadapter_release);
  if (ret < 0) pr_err("%s failed: %x", __func__, ret);
  return ret;
}
...
static int dxgk_get_device_state(struct dxgprocess *process,
  ...
  if (args.state_type == _D3DKMT_DEVICESTATE_EXECUTION) {
    global_device_state_counter = atomic_read(&dxgglobal->device_state_counter);
...
static int dxgsharedsyncobj_get_host_nt_handle(
    struct dxgsharedsyncobject *syncobj, struct dxgprocess *process,
    struct d3dkmthandle objecthandle) {
  int ret = 0;

  mutex_lock(&syncobj->fd_mutex);
  if (syncobj->host_shared_handle_nt_reference == 0) {
    ret = dxgvmb_send_create_nt_shared_object(process, objecthandle,
...
static int dxgsharedresource_get_host_nt_handle(
    struct dxgsharedresource *resource, struct dxgprocess *process,
    struct d3dkmthandle objecthandle) {
  int ret = 0;
...
enum dxg_sharedobject_type { DXG_SHARED_SYNCOBJECT, DXG_SHARED_RESOURCE };

static int get_object_fd(enum dxg_sharedobject_type type, void *object,
                         int *fdout) {
  struct file *file;
  int fd;
  ...
  switch (type) {
    case DXG_SHARED_SYNCOBJECT:
      file = anon_inode_getfile("dxgsyncobj", &dxg_syncobj_fops, object, 0);
      break;
    case DXG_SHARED_RESOURCE:
      file = anon_inode_getfile("dxgresource", &dxg_resource_fops, object, 0);
      break;
    default:
      return -EINVAL;
  };
...
static int dxgk_share_objects(struct dxgprocess *process, void *__user inargs) {
  ...
  hmgrtable_lock(&process->handle_table, DXGLOCK_SHARED);
  object_type = hmgrtable_get_object_type(&process->handle_table, handles[0]);
  ...
  switch (object_type) {
    case HMGRENTRY_TYPE_DXGSYNCOBJECT:
      ret = get_object_fd(DXG_SHARED_SYNCOBJECT, shared_syncobj, &object_fd);
      if (ret < 0) {
        pr_err("%s get_object_fd failed for sync object", __func__);
        goto cleanup;
      }
      ret = dxgsharedsyncobj_get_host_nt_handle(shared_syncobj, process,
                                                handles[0]);
      if (ret < 0) {
      ...
      break;
    case HMGRENTRY_TYPE_DXGRESOURCE:
      ret = get_object_fd(DXG_SHARED_RESOURCE, shared_resource, &object_fd);
      if (ret < 0) {
        pr_err("%s get_object_fd failed for resource", __func__);
        goto cleanup;
      }
      ret = dxgsharedresource_get_host_nt_handle(shared_resource, process,
                                                 handles[0]);
      if (ret < 0) {
...
cleanup:
  if (ret < 0) {
    if (object_fd > 0) put_unused_fd(object_fd);
  }

  if (handles) vfree(handles);

  if (syncobj) kref_put(&syncobj->syncobj_kref, dxgsyncobject_release);

  if (resource) kref_put(&resource->resource_kref, dxgresource_release);
...
static int dxgk_query_resource_info_nt(struct dxgprocess *process,
  ...
  args.private_runtime_data_size = shared_resource->runtime_private_data_size;
  ...
  args.total_priv_drv_data_size = shared_resource->alloc_private_data_size;
...
cleanup:
  ...
  if (file) fput(file);
  if (device) dxgdevice_release_lock_shared(device);
  if (device) kref_put(&device->device_kref, dxgdevice_release);
...
int assign_resource_handles(struct dxgprocess *process,
  ...
  ret = hmgrtable_assign_handle(&process->handle_table, resource,
                                HMGRENTRY_TYPE_DXGRESOURCE, resource_handle);
  if (ret < 0) goto cleanup;
  ...
  for (i = 0; i < args->allocation_count; i++) {
    ret = hmgrtable_assign_handle(&process->handle_table, allocs[i],
                                  HMGRENTRY_TYPE_DXGALLOCATION, handles[i]);
    if (ret < 0) goto cleanup;
    ...
    ret = copy_to_user(&args->open_alloc_info[i], &open_alloc_info,
...
int open_resource(struct dxgprocess *process,
  ...
  int i;
  struct d3dkmthandle *alloc_handles = NULL;
  int alloc_handles_size = sizeof(struct d3dkmthandle) * args->allocation_count;
  struct dxgsharedresource *shared_resource = NULL;
  struct dxgresource *resource = NULL;
  struct dxgallocation **allocs = NULL;
  ...
  if (shared_resource == NULL) {
    pr_err("Invalid shared resource handle: %x", (u32)args->nt_handle);
    ret = -EINVAL;
    goto cleanup;
  }
  ...
  for (i = 0; i < args->allocation_count; i++) {
    ...
    if (allocs[i] == NULL) goto cleanup;
    ret = dxgresource_add_alloc(resource, allocs[i]);
    if (ret < 0) goto cleanup;
  }

  ret = dxgvmb_send_open_resource(
      process, adapter, device->handle, global_share, args->allocation_count,
      args->total_priv_drv_data_size, &resource_handle, alloc_handles);
  if (ret < 0) {
  ...
  ret = assign_resource_handles(process, shared_resource, args, resource_handle,
                                resource, allocs, alloc_handles);
  if (ret < 0) goto cleanup;

  ret = copy_to_user(res_out, &resource_handle, sizeof(struct d3dkmthandle));
  if (ret) {
...
cleanup:
  ...
  if (ret < 0) {
    if (resource_handle.v) {
      ...
      ret = dxgvmb_send_destroy_allocation(process, device, &tmp, NULL);
    }
    if (resource) dxgresource_destroy(resource);
  }
...
static int dxgk_open_resource_nt(struct dxgprocess *process,
                                 void *__user inargs) {
  struct d3dkmt_openresourcefromnthandle args;
  ...
  ret = open_resource(process, &args, &args_user->resource,
...
static int dxgk_share_object_with_host(struct dxgprocess *process,
                                       void *__user inargs) {
...
static int dxgk_create_context(struct dxgprocess *process,
                               void *__user inargs) {
  pr_err("%s is not implemented", __func__);
  return -ENOTTY;
}
...
static int get_resource_host_nthandle(int guest_fd,
  ...
  file = fget(guest_fd);
  if (!file) {
    pr_err("failed to get file from handle: %x", guest_fd);
    ret = -EINVAL;
    goto cleanup;
  }
  if (file->f_op != &dxg_resource_fops) {
  ...
  if (kref_get_unless_zero(&shared_resource->sresource_kref) == 0) {
    pr_err("Invalid shared resource handle: %x", (u32)guest_fd);
    ret = -EINVAL;
    goto cleanup;
  }
...
static int dxgk_presentvirtual(struct dxgprocess *process,
                               void *__user inargs) {
  ...
  if (ret) {
    pr_err("%s failed to host nthandle for fd %x", __func__,
           args.composition_memory_fd);
    ret = -EINVAL;
    goto cleanup;
  }

  ret = dxgvmb_send_present_virtual(process, &args, acquire_semaphore_nthandle,
                                    release_semaphore_nthandle,
                                    composition_memory_nthandle);
...
static int dxgk_presentvirtual2(struct dxgprocess *process,
                                void *__user inargs) {
  ...
  layer_memory_nthandles = vzalloc(sizeof(u64) * args.layer_memory_fd_count);

  if (!layer_memory_nthandles) {
    pr_err("%s failed to allocate layer memory nthandles", __func__);
    ret = -ENOMEM;
    goto cleanup;
  }
  ...
  if (args.acquire_layers_semaphore_fd != -1) {
    ret = get_syncobj_host_nthandle(args.acquire_layers_semaphore_fd,
  ...
  if (args.acquire_target_semaphore_fd != -1) {
    ret = get_syncobj_host_nthandle(args.acquire_target_semaphore_fd,
  ...
  for (layer_idx = 0; layer_idx < args.layer_memory_fd_count; layer_idx++) {
    ret = get_resource_host_nthandle(layer_fds[layer_idx],
  ...
  ret = dxgvmb_send_present_virtual2(
      process, &args, acquire_layers_semaphore_nthandle,
      acquire_target_semaphore_nthandle, release_layers_semaphore_nthandle,
      release_target_semaphore_nthandle, target_memory_nthandle,
...
static int dxgk_ioctl(struct file *f, unsigned int p1, unsigned long p2) {
  ...
  if (code < 1 || code > LX_IO_MAX) {
    pr_err("bad ioctl %x %x %x %x", code, _IOC_TYPE(p1), _IOC_SIZE(p1),
           _IOC_DIR(p1));
    return -ENOTTY;
  }
  ...
  if (ioctls[code].ioctl != p1) {
    pr_err("ioctl mismatch. Code: %x User: %x Kernel: %x", code, p1,
           ioctls[code].ioctl);
    return -ENOTTY;
  }
  process = (struct dxgprocess *)f->private_data;
  if (process->tgid != current->tgid) {
    pr_err("Call from a wrong process: %d %d", process->tgid, current->tgid);
    return -ENOTTY;
  }
...
void init_ioctls(void) {
  SET_IOCTL(/*0x1 */ dxgk_open_adapter_from_luid, LX_DXOPENADAPTERFROMLUID);
  SET_IOCTL(/*0x2 */ dxgk_create_device, LX_DXCREATEDEVICE);
  SET_IOCTL(/*0x3 */ dxgk_create_context, LX_DXCREATECONTEXT);
  SET_IOCTL(/*0x4 */ dxgk_create_context_virtual, LX_DXCREATECONTEXTVIRTUAL);
  SET_IOCTL(/*0x5 */ dxgk_destroy_context, LX_DXDESTROYCONTEXT);
  SET_IOCTL(/*0x6 */ dxgk_create_allocation, LX_DXCREATEALLOCATION);
  SET_IOCTL(/*0x7 */ dxgk_create_paging_queue, LX_DXCREATEPAGINGQUEUE);
  SET_IOCTL(/*0x8 */ dxgk_reserve_gpu_va, LX_DXRESERVEGPUVIRTUALADDRESS);
  SET_IOCTL(/*0x9 */ dxgk_query_adapter_info, LX_DXQUERYADAPTERINFO);
  SET_IOCTL(/*0xa */ dxgk_query_vidmem_info, LX_DXQUERYVIDEOMEMORYINFO);
  SET_IOCTL(/*0xb */ dxgk_make_resident, LX_DXMAKERESIDENT);
  SET_IOCTL(/*0xc */ dxgk_map_gpu_va, LX_DXMAPGPUVIRTUALADDRESS);
  SET_IOCTL(/*0xd */ dxgk_escape, LX_DXESCAPE);
  SET_IOCTL(/*0xe */ dxgk_get_device_state, LX_DXGETDEVICESTATE);
  SET_IOCTL(/*0xf */ dxgk_submit_command, LX_DXSUBMITCOMMAND);
  ...
  SET_IOCTL(/*0x12 */ dxgk_wait_sync_object, LX_DXWAITFORSYNCHRONIZATIONOBJECT);
  SET_IOCTL(/*0x13 */ dxgk_destroy_allocation, LX_DXDESTROYALLOCATION2);
  SET_IOCTL(/*0x14 */ dxgk_enum_adapters, LX_DXENUMADAPTERS2);
  SET_IOCTL(/*0x15 */ dxgk_close_adapter, LX_DXCLOSEADAPTER);
  SET_IOCTL(/*0x16 */ dxgk_change_vidmem_reservation,
            LX_DXCHANGEVIDEOMEMORYRESERVATION);
  SET_IOCTL(/*0x17 */ dxgk_create_hwcontext, LX_DXCREATEHWCONTEXT);
  SET_IOCTL(/*0x18 */ dxgk_create_hwqueue, LX_DXCREATEHWQUEUE);
  SET_IOCTL(/*0x19 */ dxgk_destroy_device, LX_DXDESTROYDEVICE);
  SET_IOCTL(/*0x1a */ dxgk_destroy_hwcontext, LX_DXDESTROYHWCONTEXT);
  SET_IOCTL(/*0x1b */ dxgk_destroy_hwqueue, LX_DXDESTROYHWQUEUE);
  SET_IOCTL(/*0x1c */ dxgk_destroy_paging_queue, LX_DXDESTROYPAGINGQUEUE);
  SET_IOCTL(/*0x1d */ dxgk_destroy_sync_object,
            LX_DXDESTROYSYNCHRONIZATIONOBJECT);
  SET_IOCTL(/*0x1e */ dxgk_evict, LX_DXEVICT);
  SET_IOCTL(/*0x1f */ dxgk_flush_heap_transitions, LX_DXFLUSHHEAPTRANSITIONS);
  SET_IOCTL(/*0x20 */ dxgk_free_gpu_va, LX_DXFREEGPUVIRTUALADDRESS);
  ...
  SET_IOCTL(/*0x24 */ dxgk_invalidate_cache, LX_DXINVALIDATECACHE);
  SET_IOCTL(/*0x25 */ dxgk_lock2, LX_DXLOCK2);
  SET_IOCTL(/*0x26 */ dxgk_mark_device_as_error, LX_DXMARKDEVICEASERROR);
  SET_IOCTL(/*0x27 */ dxgk_offer_allocations, LX_DXOFFERALLOCATIONS);
  SET_IOCTL(/*0x28 */ dxgk_open_resource, LX_DXOPENRESOURCE);
  SET_IOCTL(/*0x29 */ dxgk_open_sync_object, LX_DXOPENSYNCHRONIZATIONOBJECT);
  SET_IOCTL(/*0x2a */ dxgk_query_alloc_residency,
            LX_DXQUERYALLOCATIONRESIDENCY);
  SET_IOCTL(/*0x2b */ dxgk_query_resource_info, LX_DXQUERYRESOURCEINFO);
  SET_IOCTL(/*0x2c */ dxgk_reclaim_allocations, LX_DXRECLAIMALLOCATIONS2);
  SET_IOCTL(/*0x2d */ dxgk_render, LX_DXRENDER);
  SET_IOCTL(/*0x2e */ dxgk_set_allocation_priority, LX_DXSETALLOCATIONPRIORITY);
  ...
  SET_IOCTL(/*0x37 */ dxgk_unlock2, LX_DXUNLOCK2);
  SET_IOCTL(/*0x38 */ dxgk_update_alloc_property, LX_DXUPDATEALLOCPROPERTY);
  SET_IOCTL(/*0x39 */ dxgk_update_gpu_va, LX_DXUPDATEGPUVIRTUALADDRESS);
  ...
  SET_IOCTL(/*0x3c */ dxgk_get_allocation_priority, LX_DXGETALLOCATIONPRIORITY);
  SET_IOCTL(/*0x3d */ dxgk_query_clock_calibration, LX_DXQUERYCLOCKCALIBRATION);
  SET_IOCTL(/*0x3e */ dxgk_enum_adapters3, LX_DXENUMADAPTERS3);
  SET_IOCTL(/*0x3f */ dxgk_share_objects, LX_DXSHAREOBJECTS);
  ...
  SET_IOCTL(/*0x42 */ dxgk_open_resource_nt, LX_DXOPENRESOURCEFROMNTHANDLE);
  SET_IOCTL(/*0x43 */ dxgk_query_statistics, LX_DXQUERYSTATISTICS);
  SET_IOCTL(/*0x44 */ dxgk_share_object_with_host, LX_DXSHAREOBJECTWITHHOST);
  SET_IOCTL(/*0x45 */ dxgk_create_sync_file, LX_DXCREATESYNCFILE);
  SET_IOCTL(/*0x46 */ dxgk_presentvirtual, LX_DXPRESENTVIRTUAL);
