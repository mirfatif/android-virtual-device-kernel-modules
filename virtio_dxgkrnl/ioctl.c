...
static int dxgk_presentvirtual2(struct dxgprocess *process, void *__user inargs)
{
	...
	__u64 acquire_semaphore_layers_nthandle = 0;
	__u64 acquire_semaphore_target_nthandle = 0;
	...
	ret = copy_from_user(layer_fds, &args.layer_memory_fd, sizeof(__s32) * args.layer_memory_fd_count);
	...
	layer_memory_nthandles = vzalloc(args.layer_memory_fd_count * sizeof(u64));
	...
	if (args.acquire_semaphore_layers_fd != -1) {
		ret = get_syncobj_host_nthandle(args.acquire_semaphore_layers_fd, &acquire_semaphore_layers_nthandle);
		if (ret) {
			pr_err("%s failed to host nthandle for fd %x", __func__, args.acquire_semaphore_layers_fd);
			ret = -EINVAL;
			goto cleanup;
		}
	}
	...
	// It is ok if -1 is passed for the acquire semaphore.
	if (args.acquire_semaphore_target_fd != -1) {
		ret = get_syncobj_host_nthandle(args.acquire_semaphore_target_fd, &acquire_semaphore_target_nthandle);
		if (ret) {
			pr_err("%s failed to host nthandle for fd %x", __func__, args.acquire_semaphore_target_fd);
			ret = -EINVAL;
			goto cleanup;
		}
	}
	...
	ret = get_syncobj_host_nthandle(args.release_layers_semaphore_fd, &release_layers_semaphore_nthandle);
	if (ret) {
		pr_err("%s failed to host nthandle for fd %x", __func__, args.release_layers_semaphore_fd);
		ret = -EINVAL;
		goto cleanup;
	}

	// get handle from release target semaphore file descriptor.
	ret = get_syncobj_host_nthandle(args.release_target_semaphore_fd, &release_target_semaphore_nthandle);
	if (ret) {
		pr_err("%s failed to host nthandle for fd %x", __func__, args.release_target_semaphore_fd);
		ret = -EINVAL;
		goto cleanup;
	}

	// get handle from composition target memory file descriptor.
	ret = get_resource_host_nthandle(args.target_memory_fd, &target_memory_nthandle);
	if (ret) {
		pr_err("%s failed to host nthandle for fd %x", __func__, args.target_memory_fd);
		ret = -EINVAL;
		goto cleanup;
	}
	...
	for(layer_idx = 0; layer_idx < args.layer_memory_fd_count; layer_idx++) {
		ret = get_resource_host_nthandle(layer_fds[layer_idx], &layer_memory_nthandles[layer_idx]);
		if (ret) {
			pr_err("%s failed to host nthandle for fd %x", __func__, layer_fds[layer_idx]);
			ret = -EINVAL;
			goto cleanup;
		}
	}
	...
	ret = dxgvmb_send_present_virtual2(process, &args,
		acquire_semaphore_layers_nthandle, acquire_semaphore_layers_nthandle,
        release_layers_semaphore_nthandle, release_target_semaphore_nthandle,
		target_memory_nthandle, layer_memory_nthandles);
...
void init_ioctls(void)
{
	...
	SET_IOCTL(/*0x48 */ dxgk_presentvirtual2,
		  LX_DXPRESENTVIRTUAL2);
}
