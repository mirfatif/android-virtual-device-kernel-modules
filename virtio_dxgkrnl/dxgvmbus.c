				...
				u64 *layer_memory_nthandle)
{
	...
	u32 cmd_size = sizeof(struct dxgkvmb_command_presentvirtual2) + layers_fd_size + args->private_data_size;
	u8 *write_pos;
	ret = init_message(&msg, NULL, process, cmd_size);
	if (ret)
		return ret;
	...
	command_vm_to_host_init2(&command->hdr,
				 ...
				 process->host_handle);
	command->acquire_semaphore_layers_nthandle = acquire_semaphore_layers_nthandle;
	command->acquire_semaphore_target_nthandle = acquire_semaphore_target_nthandle;
