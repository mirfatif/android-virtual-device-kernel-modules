		  ...
		  u32 sysmem_pages_rle_start_offset)
{
	...
	for (i = 0; i < args->alloc_count; i++) {
		...
		if (input_alloc->priv_drv_data_size && curr_alloc_size > 0) {
			...
			base_page = 0;
			pages_seen = 0;
			// We go 1 more loop (<=) on purpose, so we can flush out the last pending page.
				  ...
				  u32 result_size)
{
	...
	// Shouldn't happen, implies host driver issue (not giving right number of results ).
	if (result->size_list_size != args->alloc_count) {
		pr_err("send_get_allocation_size mismatch, expected: %d, found %d",
			args->alloc_count, result->size_list_size);
		ret = -EINVAL;
		goto cleanup;
	}
				  ...
				  *standard_alloc)
{
	...
	if (sysmem) {
		ret = dxgvmb_send_get_allocation_size(
			process,device, args, alloc_info, allocation_size_result,
