...
static int init_message(struct dxgvmbusmsg *msg, struct dxgadapter *adapter,
  ...
  bool use_ext_header = dxgglobal->vmbus_ver >= DXGK_VMBUS_INTERFACE_VERSION;

  if (use_ext_header) size += sizeof(struct dxgvmb_ext_header);
  msg->size = size;
...
static void free_message(struct dxgvmbusmsg *msg, struct dxgprocess *process) {
  if (msg->hdr && (char *)msg->hdr != msg->msg_on_stack) kfree(msg->hdr);
}

int ntstatus2int(struct ntstatus status) {
  if (NT_SUCCESS(status)) return (int)status.v;
  switch (status.v) {
...
static void command_vgpu_to_host_init1(
    struct dxgkvmb_command_vgpu_to_host *command,
...
static void command_vgpu_to_host_init2(
    struct dxgkvmb_command_vgpu_to_host *command, enum dxgkvmb_commandtype type,
    struct d3dkmthandle process) {
  command->command_type = type;
  command->process = process;
  command->command_id = 0;
...
static int dxgvmb_send_sync_msg_ntstatus(struct dxgvmbuschannel *channel,
  ...
  ret = dxgvmb_send_sync_msg(channel, command, cmd_size, status,
                             sizeof(struct ntstatus));
  if (ret >= 0) ret = ntstatus2int(*status);
...
static int check_iospace_address(unsigned long address, u32 size) {
  if (address < dxgglobal->mmiospace_base || size > dxgglobal->mmiospace_size ||
      address >=
          (dxgglobal->mmiospace_base + dxgglobal->mmiospace_size - size)) {
...
static u8 *dxg_map_iospace(u64 iospace_address, u32 size,
  ...
  dev_dbg(dxgglobaldev, "%s: %llx %x %lx", __func__, iospace_address, size,
          protection);
  ...
  if (vma) {
    ...
    if (!cached) prot = pgprot_writecombine(prot);
    dev_dbg(dxgglobaldev, "vma: %lx %lx %lx", vma->vm_start, vma->vm_end, va);
    vma->vm_pgoff = iospace_address >> PAGE_SHIFT;
    ret = io_remap_pfn_range(vma, vma->vm_start, vma->vm_pgoff, size, prot);
    if (ret) pr_err("io_remap_pfn_range failed: %d", ret);
  } else {
...
int dxgvmb_send_set_iospace_region(u64 start, u64 len, u32 shared_mem_gpadl) {
  ...
  command_vm_to_host_init1(&command->hdr, DXGK_VMBCOMMAND_SETIOSPACEREGION);
  command->start = start;
  command->length = len;
  command->shared_page_gpadl = shared_mem_gpadl;
  ret = dxgvmb_send_sync_msg_ntstatus(&dxgglobal->channel, msg.hdr, msg.size);
  if (ret < 0) pr_err("send_set_iospace_region failed %x", ret);
...
int dxgvmb_send_create_process(struct dxgprocess *process) {
  ...
  ret = dxgvmb_send_sync_msg(&dxgglobal->channel, msg.hdr, msg.size, &result,
                             sizeof(result));
  if (ret < 0) {
    ...
    dev_dbg(dxgglobaldev, "create_process returned %x", process->host_handle.v);
  }
...
int dxgvmb_send_open_sync_object_nt(
    struct dxgprocess *process, struct dxgvmbuschannel *channel,
  ...
  ret =
      dxgvmb_send_sync_msg(channel, msg.hdr, msg.size, &result, sizeof(result));
  ...
  if (syncobj->monitored_fence) {
    void *va = dxg_map_iospace(result.guest_cpu_physical_address, PAGE_SIZE,
    ...
    args->monitored_fence.fence_value_gpu_va = result.gpu_virtual_address;
...
int dxgvmb_send_create_nt_shared_object(struct dxgprocess *process,
  ...
  command_vm_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_CREATENTSHAREDOBJECT,
                           process->host_handle);
  ...
  ret = dxgvmb_send_sync_msg(dxgglobal_get_dxgvmbuschannel(), msg.hdr, msg.size,
                             shared_handle, sizeof(*shared_handle));
...
int dxgvmb_send_destroy_sync_object(struct dxgprocess *process,
  ...
  command_vm_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_DESTROYSYNCOBJECT,
                           process->host_handle);
...
int dxgvmb_send_share_object_with_host(
    struct dxgprocess *process, struct d3dkmt_shareobjectwithhost *args) {
  ...
  command_vm_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_SHAREOBJECTWITHHOST,
                           process->host_handle);
  ...
  if (ret || !NT_SUCCESS(result.status)) {
    if (ret == 0) ret = ntstatus2int(result.status);
    pr_err("DXGK_VMBCOMMAND_SHAREOBJECTWITHHOST failed: %d %x", ret,
           result.status.v);
    goto cleanup;
  }
...
int dxgvmb_send_present_virtual(struct dxgprocess *process,
  ...
  cmd_size =
      sizeof(struct dxgkvmb_command_presentvirtual) + args->private_data_size;

  ret = init_message(&msg, NULL, process, cmd_size);
  if (ret) return ret;
  command = (void *)msg.msg;

  command_vm_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_PRESENTVIRTUAL,
                           process->host_handle);
  ...
  if (args->private_data_size) {
    ret = copy_from_user(&command[1], args->private_data,
...
int dxgvmb_send_present_virtual2(struct dxgprocess *process,
  ...
  if (args->private_data_size) {
    ret =
        copy_from_user(write_pos, args->private_data, args->private_data_size);
    if (ret) {
      pr_err("%s failed to copy user data", __func__);
      ret = -EINVAL;
      goto cleanup;
...
struct d3dkmthandle dxgvmb_send_create_device(
    struct dxgadapter *adapter, struct dxgprocess *process,
...
struct d3dkmthandle dxgvmb_send_create_context(
    struct dxgadapter *adapter, struct dxgprocess *process,
  ...
  if (args->priv_drv_data_size) {
    ret = copy_from_user(command->priv_drv_data, args->priv_drv_data,
  ...
  ret = dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size, command, cmd_size);
  if (ret < 0) {
    goto cleanup;
  } else {
    context = command->context;
    if (args->priv_drv_data_size) {
      ret = copy_to_user(args->priv_drv_data, command->priv_drv_data,
                         args->priv_drv_data_size);
      if (ret) {
        pr_err("%s Faled to copy private data to user", __func__);
        ret = -EINVAL;
        dxgvmb_send_destroy_context(adapter, process, context);
        context.v = 0;
      }
    }
  }
...
int dxgvmb_send_create_paging_queue(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_CREATEPAGINGQUEUE,
                             process->host_handle);
  command->args = *args;
...
int dxgvmb_send_destroy_paging_queue(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_DESTROYPAGINGQUEUE,
                             process->host_handle);
...
static int copy_private_data(
  ...
  if (args->flags.standard_allocation) {
    ...
    memcpy(private_data_dest, standard_alloc, sizeof(*standard_alloc));
    private_data_dest += args->priv_drv_data_size;
  } else if (args->priv_drv_data_size) {
    ret = copy_from_user(private_data_dest, args->priv_drv_data,
                         args->priv_drv_data_size);
    if (ret) {
  ...
  if (input_alloc_info[0].sysmem) command->flags.existing_sysmem = 1;
  for (i = 0; i < args->alloc_count; i++) {
    ...
    alloc_info->priv_drv_data_size = input_alloc->priv_drv_data_size;
    /* Init to zero in case it is not sysmem. */
    alloc_info->sysmem_pages_rle_size = 0;
    if (input_alloc->priv_drv_data_size) {
      ret = copy_from_user(private_data_dest, input_alloc->priv_drv_data,
...
static int copy_sysmem_pages_rle_data(
    struct d3dkmt_createallocation *args,
    struct dxgkvmb_command_createallocation *command,
    struct d3dddi_allocationinfo *input_alloc_info,
    struct dxgkvmb_command_getallocationsize_return *allocation_size_result,
    struct dxgallocation **dxgalloc, u32 sysmem_pages_rle_limit,
  ...
  for (i = 0; i < args->alloc_count; i++) {
    ...
    if (input_alloc->priv_drv_data_size && curr_alloc_size > 0) {
      ...
      ret1 =
          get_user_pages_fast((unsigned long)input_alloc->sysmem, npages,
                              args->flags.read_only == 0, dxgalloc[i]->pages);
      if (ret1 != npages) {
        pr_err("get_user_pages_fast failed: %d", ret1);
        if (ret1 > 0 && ret1 < npages) release_pages(dxgalloc[i]->pages, ret1);
      ...
      for (pages_processed = 0; pages_processed <= npages; ++pages_processed) {
        ...
        if (pages_processed < npages - 1) ++page_in;
      }
    }
...
int create_existing_sysmem(struct dxgdevice *device,
                           ...
                           struct dxgallocation *dxgalloc, bool read_only,
                           const void *sysmem) {
  int ret1 = 0;
  void *kmem = NULL;
  int ret = 0;
  ...
  const u32 max_pfns_in_message =
      (DXG_MAX_VM_BUS_PACKET_SIZE - sizeof(*set_pages_command) - PAGE_SIZE) /
      sizeof(__u64);
  ...
  if (ret1 != npages) {
    pr_err("get_user_pages_fast failed: %d", ret1);
    if (ret1 > 0 && ret1 < npages) release_pages(dxgalloc->pages, ret1);
  ...
  if (!dxgglobal->map_guest_pages_enabled) {
    ...
    ret = dxgvmb_send_sync_msg_ntstatus(msg.channel, msg.hdr, msg.size);
    if (ret < 0) pr_err("failed to set existing store: %x", ret);
  } else {
    /*
    ...
    ret = init_message(
        &msg, device->adapter, device->process,
        sizeof(*set_pages_command) + max_pfns_in_message * sizeof(u64));
    if (ret) goto cleanup;
    ...
    while (alloc_offset_in_pages < npages) {
      pfn = (u64 *)((char *)msg.msg + sizeof(*set_pages_command));
      pages_to_send = min(npages - alloc_offset_in_pages, max_pfns_in_message);
      set_pages_command->num_pages = pages_to_send;
      set_pages_command->alloc_offset_in_pages = alloc_offset_in_pages;

      for (i = 0; i < pages_to_send; i++) *pfn++ = page_to_pfn(*page_in++);
...
static int process_allocation_handles(
    struct dxgprocess *process, struct dxgdevice *device,
    struct d3dkmt_createallocation *args,
    struct dxgkvmb_command_createallocation_return *res,
    struct dxgallocation **dxgalloc, struct dxgresource *resource) {
  int ret = 0;
  int i;
  ...
  if (args->flags.create_resource) {
    ret = hmgrtable_assign_handle(&process->handle_table, resource,
                                  HMGRENTRY_TYPE_DXGRESOURCE, res->resource);
    if (ret < 0) {
      pr_err("failed to assign resource handle %x", res->resource.v);
    } else {
  ...
  for (i = 0; i < args->alloc_count; i++) {
    ...
    ret = hmgrtable_assign_handle(&process->handle_table, dxgalloc[i],
                                  HMGRENTRY_TYPE_DXGALLOCATION,
                                  host_alloc->allocation);
    if (ret < 0) {
      pr_err("failed to assign alloc handle %x %d %d", host_alloc->allocation.v,
             args->alloc_count, i);
      break;
    }
...
static int create_local_allocations(
    struct dxgprocess *process, struct dxgdevice *device,
    struct d3dkmt_createallocation *args,
    ...
    struct dxgresource *resource, struct dxgallocation **dxgalloc,
  ...
  ret = init_message(&msg, device->adapter, process, destroy_buffer_size);
  if (ret) goto cleanup;
  ...
  for (i = 0; i < alloc_count; i++) {
    dev_dbg(dxgglobaldev, "host allocation: %d %x", i,
            result->allocation_info[i].allocation.v);
    destroy_buf->allocations[i] = result->allocation_info[i].allocation;
  }
  ...
  alloc_private_data =
      (u8 *)result + sizeof(struct dxgkvmb_command_createallocation_return) +
  ...
  for (i = 0; i < alloc_count; i++) {
    ...
    dxgalloc[i]->num_pages = host_alloc->allocation_size >> PAGE_SHIFT;
    /*
    ...
    if (host_alloc->priv_drv_data_size) {
      ret = copy_to_user(user_alloc->priv_drv_data, alloc_private_data,
    ...
    ret = copy_to_user(&args->allocation_info[i].allocation,
                       &host_alloc->allocation, sizeof(struct d3dkmthandle));
    if (ret) {
  ...
  ret = process_allocation_handles(process, device, args, result, dxgalloc,
                                   resource);
  if (ret < 0) goto cleanup;
...
cleanup:
  ...
  if (ret < 0) {
    ...
    if (dxgalloc)
      for (i = 0; i < alloc_count; i++)
        if (dxgalloc[i]) dxgallocation_free_handle(dxgalloc[i]);
    ...
    ret1 = dxgvmb_send_sync_msg_ntstatus(msg.channel, msg.hdr, msg.size);
    if (ret1 < 0) pr_err("failed to destroy allocations: %x", ret1);
...
static int get_allocation_size_private_data_copy(
    struct d3dkmt_createallocation *args,
...
int dxgvmb_send_get_allocation_size(
    struct dxgprocess *process, struct dxgdevice *device,
    struct d3dkmt_createallocation *args,
    struct d3dddi_allocationinfo *alloc_info,
    struct dxgkvmb_command_getallocationsize_return *result, u32 result_size) {
  ...
  dev_dbg(dxgglobaldev, "command size, driver_data_size %d %d %ld", cmd_size,
          priv_drv_data_size, sizeof(struct dxgkvmb_command_getallocationsize));

  ret = init_message(&msg, device->adapter, process, cmd_size);
  if (ret) goto cleanup;
  command = (void *)msg.msg;

  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_GETALLOCATIONSIZE,
                             process->host_handle);
  command->device = args->device;
  ...
  ret =
      dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size, result, result_size);
  if (ret < 0) {
...
int dxgvmb_send_create_allocation(
    struct dxgprocess *process, struct dxgdevice *device,
    struct d3dkmt_createallocation *args,
    struct d3dkmt_createallocation *__user input_args,
    struct dxgresource *resource, struct dxgallocation **dxgalloc,
  ...
  allocation_size_result_size =
      sizeof(struct dxgkvmb_command_getallocationsize_return) +
      /* Each returned size. */
  ...
  if (sysmem) {
    ret = dxgvmb_send_get_allocation_size(process, device, args, alloc_info,
                                          allocation_size_result,
                                          allocation_size_result_size);
    if (ret < 0) goto cleanup;
  }
  ...
  /*
   * Since RLE size is hard to predict, we max out the cmd size and store a
   * limit. The alternative is calculating RLE here and storing it (or
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_CREATEALLOCATION,
                             process->host_handle);
  command->device = args->device;
  command->flags = args->flags;
  ...
  if (sysmem) {
    ret = copy_sysmem_pages_rle_data(
        args, command, alloc_info, allocation_size_result, dxgalloc,
        sysmem_pages_rle_limit,
  ...
  ret =
      create_local_allocations(process, device, args, input_args, alloc_info,
                               result, resource, dxgalloc, destroy_buffer_size);

cleanup:
  if (result) vfree(result);

  if (allocation_size_result) vfree(allocation_size_result);
...
int dxgvmb_send_destroy_allocation(struct dxgprocess *process,
  ...
  destroy_buffer_size =
      sizeof(struct dxgkvmb_command_destroyallocation) + allocations_size;
  ...
  if (allocations_size)
    memcpy(destroy_buffer->allocations, alloc_handles, allocations_size);
...
int dxgvmb_send_query_clock_calibration(
    struct dxgprocess *process, struct dxgadapter *adapter,
    struct d3dkmt_queryclockcalibration *args,
    struct d3dkmt_queryclockcalibration *__user inargs) {
...
int dxgvmb_send_flush_heap_transitions(
    struct dxgprocess *process, struct dxgadapter *adapter,
...
int dxgvmb_send_query_alloc_residency(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  if (args->allocation_count) {
    alloc_size = args->allocation_count * sizeof(struct d3dkmthandle);
    cmd_size += alloc_size;
    result_allocation_size =
        args->allocation_count * sizeof(args->residency_status[0]);
  } else {
  ...
  command->args = *args;
  if (alloc_size) {
    ret = copy_from_user(&command[1], args->allocations, alloc_size);
    if (ret) {
  ...
  ret =
      copy_to_user(args->residency_status, &result[1], result_allocation_size);
  if (ret) {
...
int dxgvmb_send_escape(struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  cmd_size =
      cmd_size - sizeof(args->priv_drv_data[0]) + args->priv_drv_data_size;

  ret = init_message(&msg, adapter, process, cmd_size);
  if (ret) goto cleanup;
  command = (void *)msg.msg;
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_ESCAPE,
                             process->host_handle);
  ...
  ret = dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size,
                             command->priv_drv_data, args->priv_drv_data_size);
  if (ret < 0) goto cleanup;
...
int dxgvmb_send_query_vidmem_info(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  ret = copy_to_user(&output->budget, &result.budget, sizeof(output->budget));
  if (ret) {
  ...
  ret = copy_to_user(&output->current_reservation, &result.current_reservation,
...
int dxgvmb_send_get_device_state(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, dxgk_vmbcommand_getdevicestate,
                             process->host_handle);
  command->args = *args;
...
int dxgvmb_send_open_resource(
    struct dxgprocess *process, struct dxgadapter *adapter,
    struct d3dkmthandle device, struct d3dkmthandle global_share,
    u32 allocation_count, u32 total_priv_drv_data_size,
    struct d3dkmthandle *resource_handle, struct d3dkmthandle *alloc_handles) {
  ...
  int i;
  u32 result_size =
      allocation_count * sizeof(struct d3dkmthandle) + sizeof(*result);
  struct dxgvmbusmsgres msg = {.hdr = NULL};

  ret = init_message_res(&msg, adapter, process, sizeof(*command), result_size);
  if (ret) goto cleanup;
  command = msg.msg;
  result = msg.res;
  ...
  *resource_handle = result->resource;
  ...
  for (i = 0; i < allocation_count; i++) alloc_handles[i] = handles[i];
...
int dxgvmb_send_get_stdalloc_data(
    struct dxgdevice *device, enum d3dkmdt_standardallocationtype alloctype,
    struct d3dkmdt_gdisurfacedata *alloc_data, u32 physical_adapter_index,
    u32 *alloc_priv_driver_size, void *priv_alloc_data, u32 *res_priv_data_size,
    void *priv_res_data) {
  ...
  if (priv_alloc_data) result_size += *alloc_priv_driver_size;
  if (priv_res_data) result_size += *res_priv_data_size;
  ...
  *res_priv_data_size = result->priv_driver_resource_size;
  if (priv_alloc_data) {
    memcpy(priv_alloc_data, &result[1], result->priv_driver_data_size);
  }
  if (priv_res_data) {
    memcpy(priv_res_data, (char *)(&result[1]) + result->priv_driver_data_size,
...
int dxgvmb_send_make_resident(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_MAKERESIDENT,
                             process->host_handle);
  command->alloc_count = args->alloc_count;
...
int dxgvmb_send_evict(struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_EVICT,
                             process->host_handle);
  command->alloc_count = args->alloc_count;
  command->device = args->device;
  command->flags = args->flags;
...
int dxgvmb_send_submit_command(struct dxgprocess *process,
  ...
  cmd_size = sizeof(struct dxgkvmb_command_submitcommand) + hbufsize +
             args->priv_drv_data_size;
  ...
  ret = copy_from_user(&command[1], args->history_buffer_array, hbufsize);
  if (ret) {
    pr_err("%s failed to copy history buffer", __func__);
    ret = -EINVAL;
    goto cleanup;
  }
  ret = copy_from_user((u8 *)&command[1] + hbufsize, args->priv_drv_data,
                       args->priv_drv_data_size);
  if (ret) {
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_SUBMITCOMMAND,
                             process->host_handle);
  command->args = *args;
...
int dxgvmb_send_update_gpu_va(struct dxgprocess *process,
  ...
  if (args->num_operations == 0 ||
      (DXG_MAX_VM_BUS_PACKET_SIZE /
       sizeof(struct d3dddi_updategpuvirtualaddress_operation)) <
          args->num_operations) {
    ret = -EINVAL;
    pr_err("Invalid number of operations: %d", args->num_operations);
    goto cleanup;
  }

  op_size = args->num_operations *
            sizeof(struct d3dddi_updategpuvirtualaddress_operation);
  cmd_size = sizeof(struct dxgkvmb_command_updategpuvirtualaddress) + op_size -
             sizeof(args->operations[0]);
  ...
  ret = copy_from_user(command->operations, args->operations, op_size);
  if (ret) {
...
static void set_result(struct d3dkmt_createsynchronizationobject2 *args,
  ...
  args->info.periodic_monitored_fence.fence_gpu_virtual_address = fence_gpu_va;
  args->info.periodic_monitored_fence.fence_cpu_virtual_address = va;
}

int dxgvmb_send_create_sync_object(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_CREATESYNCOBJECT,
                             process->host_handle);
  command->args = *args;
  ...
  args->sync_object = result.sync_object;
  if (syncobj->shared) args->info.shared_handle = result.global_sync_object;
  ...
  if (syncobj->monitored_fence) {
    ...
    if (args->info.type == _D3DDDI_MONITORED_FENCE) {
      args->info.monitored_fence.fence_gpu_virtual_address =
          result.fence_gpu_va;
      args->info.monitored_fence.fence_cpu_virtual_address = va;
      {
        unsigned long value;

        dev_dbg(dxgglobaldev, "fence cpu va: %p", va);
        ret = copy_from_user(&value, va, sizeof(u64));
        if (ret) {
          pr_err("failed to read fence");
          ret = -EINVAL;
        } else {
          dev_dbg(dxgglobaldev, "fence value:%lx", value);
        }
      }
    } else {
...
int dxgvmb_send_signal_sync_object(
    struct dxgprocess *process, struct dxgadapter *adapter,
    struct d3dddicb_signalflags flags, u64 legacy_fence_value,
    struct d3dkmthandle context, u32 object_count,
    struct d3dkmthandle __user *objects, u32 context_count,
    struct d3dkmthandle __user *contexts, u32 fence_count, u64 __user *fences,
    struct eventfd_ctx *cpu_event_handle, struct d3dkmthandle device,
    bool user_address) {
  int ret;
  ...
  u8 *current_pos;
  u32 cmd_size = sizeof(struct dxgkvmb_command_signalsyncobject) + object_size +
                 context_size + fence_size;
  struct dxgvmbusmsg msg = {.hdr = NULL};

  if (context.v) cmd_size += sizeof(struct d3dkmthandle);

  ret = init_message(&msg, adapter, process, cmd_size);
  if (ret) goto cleanup;
  command = (void *)msg.msg;

  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_SIGNALSYNCOBJECT,
                             process->host_handle);
  ...
  if (user_address) {
    ...
    if (ret) {
      pr_err("Failed to read objects %p %d", objects, object_size);
      ret = -EINVAL;
      goto cleanup;
    }
  } else {
  ...
  if (context_size) {
    if (user_address) {
      ...
      if (ret) {
        pr_err("Failed to read contexts %p %d", contexts, context_size);
        ret = -EINVAL;
        goto cleanup;
      }
    } else {
  ...
  if (fence_size) {
    if (user_address) {
      ...
      if (ret) {
        pr_err("Failed to read fences %p %d", fences, fence_size);
        ret = -EINVAL;
        goto cleanup;
      }
    } else {
...
int dxgvmb_send_wait_sync_object_cpu(
    struct dxgprocess *process, struct dxgadapter *adapter,
    struct d3dkmt_waitforsynchronizationobjectfromcpu *args, bool user_address,
    u64 cpu_event) {
  int ret = -EINVAL;
...
int dxgvmb_send_wait_sync_object_gpu(struct dxgprocess *process,
                                     ...
                                     struct d3dkmthandle context,
                                     u32 object_count,
                                     struct d3dkmthandle *objects, u64 *fences,
                                     bool legacy_fence) {
  int ret;
...
int dxgvmb_send_lock2(struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_LOCK2,
                             process->host_handle);
  command->args = *args;
  ...
  if (alloc == NULL) {
    ...
    if (alloc->cpu_address) {
      ...
      args->data = dxg_map_iospace(offset, alloc->num_pages << PAGE_SHIFT,
    ...
    if (args->data == NULL) {
      ...
      ret = copy_to_user(&outargs->data, &args->data, sizeof(args->data));
      if (ret) {
...
int dxgvmb_send_unlock2(struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_UNLOCK2,
                             process->host_handle);
  command->args = *args;
...
int dxgvmb_send_update_alloc_property(
    struct dxgprocess *process, struct dxgadapter *adapter,
    struct d3dddi_updateallocproperty *args,
    struct d3dddi_updateallocproperty *__user inargs) {
  int ret;
  int ret1;
  ...
  if (ret == STATUS_PENDING) {
    ret1 = copy_to_user(&inargs->paging_fence_value, &result.paging_fence_value,
                        sizeof(u64));
    if (ret1) {
...
int dxgvmb_send_mark_device_as_error(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_MARKDEVICEASERROR,
                             process->host_handle);
  command->args = *args;
  ret = dxgvmb_send_sync_msg_ntstatus(msg.channel, msg.hdr, msg.size);
cleanup:
...
int dxgvmb_send_set_allocation_priority(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  ret = copy_from_user(allocations, args->allocation_list, alloc_size);
  if (ret) {
    pr_err("%s failed to copy alloc handle", __func__);
    ret = -EINVAL;
    goto cleanup;
  }
  ret = copy_from_user((u8 *)allocations + alloc_size, args->priorities,
                       priority_size);
  if (ret) {
...
int dxgvmb_send_get_allocation_priority(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  ret = dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size + msg.res_size,
                             result, msg.res_size);
  if (ret < 0) goto cleanup;
  ...
  ret = copy_to_user(args->priorities, (u8 *)result + sizeof(*result),
                     priority_size);
  if (ret) {
...
int dxgvmb_send_offer_allocations(struct dxgprocess *process,
  ...
  u32 cmd_size = sizeof(struct dxgkvmb_command_offerallocations) + alloc_size -
                 sizeof(struct d3dkmthandle);
  struct dxgvmbusmsg msg = {.hdr = NULL};
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_OFFERALLOCATIONS,
                             process->host_handle);
  command->flags = args->flags;
  ...
  if (args->resources) {
    ...
    ret = copy_from_user(command->allocations, args->resources, alloc_size);
  } else {
    ret = copy_from_user(command->allocations, args->allocations, alloc_size);
  }
  if (ret) {
...
int dxgvmb_send_reclaim_allocations(struct dxgprocess *process,
  ...
  if (args->results)
    result_size +=
        (args->allocation_count - 1) * sizeof(enum d3dddi_reclaim_result);
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_RECLAIMALLOCATIONS,
                             process->host_handle);
  command->device = device;
  command->paging_queue = args->paging_queue;
  command->allocation_count = args->allocation_count;
  ...
  ret = dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size, result,
                             msg.res_size);
  if (ret < 0) goto cleanup;
  ret = copy_to_user(paging_fence_value, &result->paging_fence_value,
                     sizeof(u64));
  if (ret) {
    pr_err("%s failed to copy paging fence", __func__);
    ret = -EINVAL;
  ...
  if (NT_SUCCESS(result->status) && args->results) {
    ret = copy_to_user(args->results, result->discarded,
                       sizeof(result->discarded[0]) * args->allocation_count);
    if (ret) {
...
int dxgvmb_send_change_vidmem_reservation(
    struct dxgprocess *process, struct dxgadapter *adapter,
...
int dxgvmb_send_create_hwqueue(struct dxgprocess *process,
  ...
  if (args->priv_drv_data_size) cmd_size += args->priv_drv_data_size - 1;

  ret = init_message(&msg, adapter, process, cmd_size);
  if (ret) goto cleanup;
  command = (void *)msg.msg;

  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_CREATEHWQUEUE,
                             process->host_handle);
  command->context = args->context;
  command->flags = args->flags;
  command->priv_drv_data_size = args->priv_drv_data_size;
  ...
  ret =
      hmgrtable_assign_handle_safe(&process->handle_table, hwqueue,
                                   HMGRENTRY_TYPE_DXGHWQUEUE, command->hwqueue);
  if (ret < 0) goto cleanup;

  ret = hmgrtable_assign_handle_safe(&process->handle_table, NULL,
  ...
  hwqueue->progress_fence_mapped_address =
      dxg_map_iospace((u64)command->hwqueue_progress_fence_cpuva, PAGE_SIZE,
                      PROT_READ | PROT_WRITE, true);
  ...
  ret = copy_to_user(&inargs->queue_progress_fence_gpu_va,
                     &command->hwqueue_progress_fence_gpuva, sizeof(u64));
  if (ret) {
  ...
  if (ret < 0) {
    ...
    if (hwqueue->handle.v) {
      hmgrtable_free_handle_safe(&process->handle_table,
                                 HMGRENTRY_TYPE_DXGHWQUEUE, hwqueue->handle);
      hwqueue->handle.v = 0;
    }
    if (command && command->hwqueue.v)
      dxgvmb_send_destroy_hwqueue(process, adapter, command->hwqueue);
  }
  free_message(&msg, process);
  return ret;
}
...
int dxgvmb_send_query_adapter_info(struct dxgprocess *process,
  ...
  ret = copy_from_user(command->private_data, args->private_data,
                       args->private_data_size);
  if (ret) {
    pr_err("%s Faled to copy private data", __func__);
    ret = -EINVAL;
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_QUERYADAPTERINFO,
                             process->host_handle);
  command->private_data_size = args->private_data_size;
  ...
  if (dxgglobal->vmbus_ver >= DXGK_VMBUS_INTERFACE_VERSION) {
    ...
    private_data_size = command->private_data_size + sizeof(struct ntstatus);
  } else {
    private_data = command->private_data;
    private_data_size = command->private_data_size;
  }

  ret = dxgvmb_send_sync_msg(msg.channel, msg.hdr, msg.size, private_data,
                             private_data_size);
  if (ret < 0) goto cleanup;
  ...
  switch (args->type) {
    ...
    case _KMTQAITYPE_ADAPTERTYPE_RENDER: {
      struct d3dkmt_adaptertype *adapter_type = (void *)private_data;
  ...
  ret = copy_to_user(args->private_data, private_data, args->private_data_size);
  if (ret) {
    pr_err("%s Faled to copy private data to user", __func__);
    ret = -EINVAL;
  }
...
int dxgvmb_send_submit_command_hwqueue(
    struct dxgprocess *process, struct dxgadapter *adapter,
  ...
  if (primaries_size) {
    ret = copy_from_user(&command[1], args->written_primaries, primaries_size);
    if (ret) {
...
int dxgvmb_send_query_statistics(struct dxgprocess *process,
  ...
  command_vgpu_to_host_init2(&command->hdr, DXGK_VMBCOMMAND_QUERYSTATISTICS,
                             process->host_handle);
  command->args = *args;
