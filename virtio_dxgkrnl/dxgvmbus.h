...
enum dxgkvmb_commandtype_global {
	...
	DXGK_VMBCOMMAND_PRESENTVIRTUAL2		= 1032,
...
struct dxgkvmb_command_presentvirtual2 {
	struct dxgkvmb_command_vm_to_host hdr;
	u64 acquire_semaphore_nthandle; // wait semaphore for acquisition
	...
	u32 target_memory_nthandle; // Target to composite into
	u32 layer_fd_count; // Layers fd count
	u64 private_data_size; // Nexus private data size
	u64 layer_fds[1]; // Flexible array member for layer file descriptors
	u8 private_data[1]; // Flexible array member for private data
};
