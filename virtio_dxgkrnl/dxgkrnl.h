...
struct dxgsharedsyncobject *dxgsharedsyncobj_create(
    struct dxgadapter *adapter, struct dxgsyncobject *syncobj);
...
struct dxgsyncobject *dxgsyncobject_create(
    struct dxgprocess *process, struct dxgdevice *device,
    struct dxgadapter *adapter, enum d3dddi_synchronizationobject_type type,
...
struct dxgprocess_adapter *dxgprocess_adapter_create(
...
struct dxgprocess_adapter *dxgprocess_get_adapter_info(
    struct dxgprocess *process, struct dxgadapter *adapter);
...
int dxgvmb_send_create_sync_object(
    struct dxgprocess *pr, struct dxgadapter *adapter,
    struct d3dkmt_createsynchronizationobject2 *args, struct dxgsyncobject *so);
...
int dxgvmb_send_signal_sync_object(
    ...
    struct d3dkmthandle context, u32 object_count, struct d3dkmthandle *object,
    u32 context_count, struct d3dkmthandle *contexts, u32 fence_count,
    u64 *fences, struct eventfd_ctx *cpu_event, struct d3dkmthandle device,
    bool user_address);
...
int dxgvmb_send_get_stdalloc_data(struct dxgdevice *device,
                                  ...
                                  u32 *res_priv_data_size, void *priv_res_data);
