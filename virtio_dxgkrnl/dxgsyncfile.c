...
static void dxgdmafence_release(struct dma_fence *fence)
{
	...
	struct dxg_sync_cb_t *entry, *tmp;

	syncpoint = to_syncpoint(fence);
	if (syncpoint) {
		if (syncpoint->hdr.event_id)
			dxgglobal_get_host_event(syncpoint->hdr.event_id);
		// free up allocated memory for sync objects and fence values.
		list_for_each_entry_safe(entry, tmp, &syncpoint->sync_cb_list, node) {
			if (entry) {
				// delete callback entry from list
				list_del(&entry->node);
				// free sync objects/fence values.
				if (entry->objects) {
					kfree(entry->objects);
				}
				if (entry->fence_values) {
					kfree(entry->fence_values);
				}
				kfree(entry);
			}
		}
