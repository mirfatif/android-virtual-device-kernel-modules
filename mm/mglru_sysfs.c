...
void notify_mglru_gen(void* data, void* unused) {
...
static int __init init_mglru_sysfs(void)
{
	...
	lru_gen_admin_node = kernfs_find_and_get(mm_kobj->sd, "lru_gen_admin");

	ret = register_trace_android_vh_mglru_new_gen(
		notify_mglru_gen, NULL);
