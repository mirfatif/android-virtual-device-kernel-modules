...
enum virtwl_ioctl_new_type {
	VIRTWL_IOCTL_NEW_CTX = 0, /* open a new wayland connection context */
...
#define VIRTIO_WL_CTX_NAME_MAX_LEN 32
...
struct virtwl_ioctl_new {
	...
	union {
		...
		/* name of context if type == VIRTIO_WL_CMD_VFD_NEW_CTX_NAMED */
		__u8 name[VIRTIO_WL_CTX_NAME_MAX_LEN];
	};
};
